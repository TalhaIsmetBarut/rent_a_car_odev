...
$kontrol_sql = "SELECT * FROM kullanicilar WHERE kullanici_adi = '$kullanici_adi'";
$kontrol_result = mysqli_query($conn, $kontrol_sql);

if ($kontrol_result && mysqli_num_rows($kontrol_result) > 0) {
    header("Location: index.php?sayfa=kayit&hata=" . urlencode("Bu kullanıcı adı zaten alınmış."));
    exit;
}


$sql = "INSERT INTO kullanicilar (kullanici_adi, eposta, sifre, rol) VALUES ('$kullanici_adi', '$eposta', '$hashed_sifre', 'user')";
...
if ($ornek) {
    echo "<div class='alert alert-success d-flex align-items-center mb-4'><i class='bi bi-check-circle-fill fs-4 me-2'></i> <div><strong>Başarılı!</strong> Kaydınız başarıyla oluşturuldu.</div></div>";
} else {
...
<div class="card p-3 shadow-sm mt-3">
    <div class="card-body py-2">
        Giriş yapmak için <a href="?sayfa=giris" class="btn btn-outline-primary btn-sm ms-2"><i class="bi bi-box-arrow-in-right me-1"></i> Tıklayınız</a>
    </div>
</div>
